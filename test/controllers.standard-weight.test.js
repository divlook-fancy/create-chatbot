...
describe('Controllers', () => {
  describe('StandardWeight', () => {
    ...
    describe('function chatStart', () => {
      ...
      before(() => {
        sinon.stub(defaultData, 'param').value({
          user_key: 'user_key',
          type: 'text',
          content: 'content',
        })
        sinon
          ...
          .callsFake(arg => {
            switch (arg) {
              case `event:${defaultData.param.user_key}`:
                return event
                break
              case `eventClass:${defaultData.param.user_key}`:
                return eventClass
                break
            }
          })
      ...
      describe('# 키워드가 없을 때', () => {
        it('아무거나', () => {
          sinon.stub(defaultData.param, 'content').value('아무거나')
          should(chatStart(serverData, defaultData)).is.False()
        })
        it('날씨 어때?', () => {
          sinon.stub(defaultData.param, 'content').value('날씨 어때?')
          should(chatStart(serverData, defaultData)).is.False()
        })
      })
      describe('# 키워드가 있을 때', () => {
        ...
        it('체중계', () => {
          sinon.stub(defaultData.param, 'content').value('체중계')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('몸무게', () => {
          sinon.stub(defaultData.param, 'content').value('몸무게')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('비만', () => {
          sinon.stub(defaultData.param, 'content').value('비만')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
      })
      describe('# 첫번째 이벤트 : 표준몸무게 알아볼까?', () => {
        ...
        it('좋아', () => {
          sinon.stub(defaultData.param, 'content').value('좋아')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('아니', () => {
          sinon.stub(defaultData.param, 'content').value('아니')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('안해', () => {
          sinon.stub(defaultData.param, 'content').value('안해')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
      })
      describe('# 두번째 이벤트 : 키를 입력해줘', () => {
        ...
        it('숫자입력 = 180', () => {
          sinon.stub(defaultData.param, 'content').value(180)
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('문자입력', () => {
          sinon.stub(defaultData.param, 'content').value('백팔십')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
      })
      describe('# 두번째 이벤트 : 몸무게를 입력해줘', () => {
        ...
        it('숫자입력 = 70', () => {
          sinon.stub(defaultData.param, 'content').value(70)
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('문자입력', () => {
          sinon.stub(defaultData.param, 'content').value('칠십')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
      })
      describe('# 세번째 이벤트 : 성별을 입력해줘', () => {
        ...
        it('남자', () => {
          sinon.stub(defaultData.param, 'content').value('남자')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
        it('여자', () => {
          sinon.stub(defaultData.param, 'content').value('여자')
          should(chatStart(serverData, defaultData)).is.not.False()
        })
      })
      describe('# 결과', () => {
